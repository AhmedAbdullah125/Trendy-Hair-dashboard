...
const AdminBrands: React.FC = () => {
  ...
  const [editingBrand, setEditingBrand] = useState<Partial<BrandFormData> | null>(null);
  ...
  const { data, isLoading, error, refetch } = useGetAdminBrands(pageSize, currentPage, 'ar');

  // Mutations
  const addMutation = useAddAdminBrand();
  const updateMutation = useUpdateAdminBrand();
  const deleteMutation = useDeleteAdminBrand();
  ...
  const filteredBrands = brands.filter((brand: any) =>
    brand.name_ar?.toLowerCase().includes(search.toLowerCase()) ||
    brand.name_en?.toLowerCase().includes(search.toLowerCase())
  );

  const openAddModal = () => {
    const maxSortOrder = brands.length > 0 ? Math.max(...brands.map((b: any) => b.position || 0)) : 0;
  ...
  const handleSave = async () => {
    ...
    if (editingBrand.id) {
      await updateMutation.mutateAsync({
        id: editingBrand.id,
        name_ar: editingBrand.name,
        name_en: editingBrand.nameEn,
        position: editingBrand.sortOrder,
        is_active: editingBrand.isActive ? 1 : 0,
  ...
  const confirmDelete = async () => {
    if (brandToDelete) {
      await deleteMutation.mutateAsync(brandToDelete);
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="py-20 text-center">
              ...
              <p className="mt-4 text-app-textSec">جاري تحميل العلامات التجارية...</p>
            </div>
          ) : error ? (
            <div className="py-20 text-center">
              <p className="text-red-500 font-bold">حدث خطأ في تحميل العلامات التجارية</p>
              <button
                onClick={() => refetch()}
              ...
              <tbody className="divide-y divide-app-card/30 text-sm">
                {filteredBrands.map((brand: any) => (
                  <tr key={brand.id} className="hover:bg-app-bg/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-app-textSec font-medium">{brand.name_en || '-'}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-lg text-xs font-bold ${brand.is_active ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
        ...
        {!isLoading && !error && pagination.total_pages > 1 && (
          <div className="p-4 border-t border-app-card/30 flex items-center justify-between bg-app-bg/30">
            <div className="text-sm text-app-textSec">
              صفحة {pagination.current_page} من {pagination.total_pages} • إجمالي العلامات: {pagination.total_items}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              {/* Image Upload */}
              <div>
                <label className="block text-sm font-bold text-app-text mb-2">شعار العلامة التجارية</label>
                <div className="space-y-3">
                  ...
                  {(editingBrand.image || editingBrand.imageUrl) && (
                    <div className="relative w-32 h-32 border-2 border-app-card rounded-xl overflow-hidden">
                      <img
                        src={editingBrand.image ? URL.createObjectURL(editingBrand.image) : editingBrand.imageUrl}
                        alt="Preview"
            ...
            <div className="p-6 border-t border-app-card/30 flex justify-end gap-3 bg-gray-50 rounded-b-2xl">
              ...
              <button
                onClick={handleSave}
                disabled={!editingBrand.name || addMutation.isPending || updateMutation.isPending}
              ...
              >
                ...
                <span>{editingBrand.id ? 'تحديث' : 'حفظ'}</span>
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-slideUp">
            ...
            <div className="p-6 text-center">
              <p className="text-app-text font-bold text-lg mb-2">هل أنت متأكد من حذف هذه العلامة التجارية؟</p>
